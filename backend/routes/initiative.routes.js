...
router.route("/")
    ...
    .post(verifyJWT, (req, res, next) => {
        console.log("Inside Initiative Route");
        console.log("Req Body:", req.body);
        console.log("Req Files:", req.files);
        next();
    }, upload.fields([{ name: "images", maxCount: MAXIMUM_INITIATIVE_IMAGE_COUNT }]), createInitiativeValidator(), validate, createInitiative);

router.route("/get/my").get(verifyJWT, getMyInitiatives);

router.route("/get/t/:tag")
    .get(
...
router.route("/:initiativeId")
    ...
    .patch(
        verifyJWT,
        upload.fields([{ name: "images", maxCount: MAXIMUM_INITIATIVE_IMAGE_COUNT }]),
        mongoIdPathVariableValidator("initiativeId"),
        updateInitiativeValidator(),
        validate,
        updateInitiative
    )
    .delete(verifyJWT, mongoIdPathVariableValidator("initiativeId"), validate, deleteInitiative);

router.route("/remove/image/:initiativeId/:imageId")
    .patch(
        verifyJWT,
        mongoIdPathVariableValidator("initiativeId"),
