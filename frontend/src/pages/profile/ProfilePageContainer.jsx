...
const ProfilePageContainer = () => {
  ...
  const clickHandler = async () => {
    try {
      setLoading(true);
      const response = await ProfileService.followUnFollowUser(profile.account._id);
      if (response instanceof ApiError) {
  ...
  return (
    ...
    <div className="max-w-4xl mx-auto mt-10 p-4 bg-white rounded-lg shadow-md">
      <div className="relative">
        <img src={profile.coverImage.url} alt={`${profile.name}'s cover`} className="w-full h-48 object-cover rounded-md" />
        <div className="absolute top-20 left-4">
          <img src={avatar} alt={`${profile.name}'s avatar`} className="w-40 h-40 rounded-full border-4 border-white" />
        </div>
      </div>
      <div className="mt-20 p-4">
        ...
        <div className="flex items-center text-gray-600 mt-2">
          <svg className="w-5 h-5" fill="#000000" version="1.1" id="Capa_1"  
      ...
      {isCurrentUser ? (
       ...
       <button
          className={`mt-2 px-4 py-2 rounded ${profile.isFollowing ? 'bg-red-500' : 'bg-blue-500'} text-white flex items-center`}
